...

{
   ...
   $data=array('email' => trim($info['email']),

		'username' => trim($info['username']),

		'password' => isset($info['password']) ? hashedPassword($info['password']) : '',

		'view_count' => trim($info['view_count']),

		'coins' => trim($info['coins']),

		'level' => trim($info['level']),

		'created_at' => trim($info['created_at']),

		'userprofile' => trim($info['userprofile']),  
	...
	$lastid=addNew('users',$data);
	if(isset($_SESSION["invite_userid"])){
		$findData = array();
				$findData = getByWhere('users', '*', array('id' => $_SESSION["invite_userid"]));
				$coins = $findData[0]->coins;
				$coins = $coins + 100;
			   updateByWhere('users', array('coins' => $coins), array('id' => $_SESSION["invite_userid"]));
			   unset($_SESSION["invite_userid"]);
		}
	return $lastid;
